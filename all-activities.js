...
fetch('data/special-events.json', { cache: 'no-store' }).then(response => response.json()).then(data => {
  ...
  eventList.innerHTML = events.length ? events.map(event => {
    ...
    return `<article class="special-event">${label ? `<p class="eyebrow">${label} · ${ages}</p>` : `<p class="eyebrow">${ages}</p>`}<h3>${event.title}</h3><p class="event-time">${timing}</p>${event.location ? `<p>${event.location}</p>` : ''}<p class="event-note">${event.confidence}</p><div class="special-actions"><a class="navigate-button" href="${routeUrl}">路线 ↗</a><a class="source" href="${event.url}" target="_blank" rel="noreferrer">查看主办方信息 ↗</a></div></article>`;
